...
template <class KEY, class VALUE> inline VALUE DsHash<KEY,VALUE>::Get (const KEY& key)
{
 int h = Hash(key);
 int i = FindKey(h,key);
...
template <class KEY, class VALUE> inline bool DsHash<KEY,VALUE>::Find (const KEY& key)
{
 int h = Hash(key);
 int i = FindKey(h,key);
