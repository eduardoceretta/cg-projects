...
public:
  enum Dimension
  {
    DIM_2D = PtcPointSet::DIM_2D,
    DIM_3D = PtcPointSet::DIM_2D
  };
...
{
  return (PtcModel::Dimension) m_pointset->GetDimension();
}
...
inline const float* PtcModel::GetVectorField () const
{
  return m_pointset->GetVectorField();
}
...
inline PtcPointSet* PtcModel::GetPointSet () const
{
  return m_pointset;
}
