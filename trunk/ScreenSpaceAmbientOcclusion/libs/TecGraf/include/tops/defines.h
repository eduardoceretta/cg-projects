...
/**
  Defines the number of bits used to store reverse adjacency information.

  The value of (TOP_REV_ADJ_FACET_BITS + TOP_REV_ADJ_EDGE_BITS) must be
  less or equal to 8, as one unsigned char per element facet is used to
  store this information, for the static topology elements.

  When using reverse adjacency, the constants TOP_MAX_NUM_F and TOP_MAX_NUM_E 
  must also be defined so they are less or equal the maximum value that can be 
  stored in the defined reverse adjacency bits.

  Reverse adjacency per facet (8 bits): { 4:rev_edge | 4:rev_facet }
*/
#define TOP_REV_ADJ_FACET_BITS 4
#define TOP_REV_ADJ_EDGE_BITS 4

#define TOP_REV_ADJ_FACET_PAT 0xf
#define TOP_REV_ADJ_EDGE_PAT 0xf0
