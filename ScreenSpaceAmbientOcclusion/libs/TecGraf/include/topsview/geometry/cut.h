...
private:
  ...
  void SortPointsInPlane(
    int np, AlgVector* pos, int* ids, int* np_out, int* ids_out);

  int IntersectLinePlane(const AlgVector& p0, const AlgVector& dir, 
    const AlgPlane& plane, float* t, AlgVector* p);
