...
int INTERSECTCLASS::GetBlockVerticesCB (float** coords, 
                                        ...
                                        void* data)
{
  GeomInfoCBData* cbdata = (GeomInfoCBData*)data;
  DsArray<int>** block_polys = cbdata->block_polys;
  POLYMODELCLASS* poly_model = cbdata->poly_model;

  DsArray<int>* block = block_polys[block_i];
  int elem_count = block->Size();
  int array_size = 0;
  // compute array size
  for (int e = 0; e < elem_count; e++) {
    int poly_i = block->Get(e);
    array_size += 3*poly_model->GetVertexCount(poly_i);
  }
  // create array
  DsArray<float> vcoords;
  vcoords.Resize(array_size, true);
  float* array_p = vcoords.GetArray();
  // read vertices coordinates
  for (int e = 0; e < elem_count; e++) {
    int poly_i = block->Get(e);
    int n = poly_model->GetVertexCount(poly_i);
    for (int v = 0; v < n; v++) {
      poly_model->GetVertexCoords(array_p, poly_i, v);
      array_p += 3;
    }
  }
  assert(array_p == vcoords.GetArrayConst() + array_size);
  int vertex_count = array_size/3;
  *coords = vcoords.TakeOverArray();
  return vertex_count;
}
...
{
  int n_blocks = m_poly_model.GetNumBlocks();
  DsArray<int>** block_polys = new DsArray<int>*[n_blocks]; // polys indexes in each block
  for (int i = 0; i < n_blocks; i++)
    block_polys[i] = new DsArray<int>;

  // find out which block each poly should lay in
  for (int poly_i = 0; poly_i < m_poly_model.GetPolyCount(); poly_i++) {
    int index = m_poly_model.Index(poly_i);
    assert(0 <= index && index < n_blocks);
    block_polys[index]->Append(poly_i);
  }
...
#define HIERARCHY_STORE_VERTICES 0
  ...
  GeomInfoCBData cbdata;
  cbdata.poly_model = &m_poly_model;
  cbdata.block_polys = block_polys;
  GeomInfoCallback cb;
  cb.SetVertexCoordsCB(INTERSECTCLASS::GetBlockVerticesCB, &cbdata);
...
#if !HIERARCHY_STORE_VERTICES
  for (int i = 0; i < n_blocks; i++)
    if (block_polys[i] != NULL)
      delete block_polys[i];
#endif
  delete [] block_polys;
}
                                              ...
                                              bool is_infinite_plane)
{
  ...
  for (unsigned int geom_i = 0; geom_i < geom_vec.size(); geom_i++) {
    ...
    const DsArray<int>& block = m_poly_model.GetBlockPolys(block_index);
    // polys in this block should be checked against cut plane
    for (int i = 0; i < block.Size(); i++) {
      int poly_index = block[i];
