...
private:
  ...
  // font cache
  FMFontCache* m_font_cache[UFONT_MANAGER_CACHE_SIZE];
  DsHash<DsStringKey,FMFontCache*> m_font_cache_hash;
  int m_font_cache_ts;

  const char* GetRenderModeName (int render_mode);
  void ComputeCacheKey (const char* key, int size, float depth, int render_mode, char cache_key[128]);
  void FontCacheInit ();
  UFontMessage* FontCacheGetMessage (const char* key, int size, float depth, int render_mode);
  void FontCacheUsingMessage (UFontMessage *m, const char* key, int size, float depth, int render_mode);
  void FontCacheDeleteMessage (const char *key, int size, float depth, int render_mode);
  static void CreateTTFont (const char* family_name);
  static void CreateGlutFont (const char* family_name);
  static void UpdateTTFontsList ();
};
