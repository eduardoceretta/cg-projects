...
private:
  ...
  *************************/
  ...
  TopElement* GetAdjArray(const TopElement& el) const
  {
  ...
  unsigned char* GetRevAdjArray(const TopElement& el) const
  {
#ifdef TOP_USE_DYNAMIC_ELEMENTS
    if (m_T[el.GetType()].IS_DYNAMIC)
      return *((unsigned char**) 
        ((unsigned char*) GetConcreteElement(el.GetHandle()) +
          m_ST[el.GetType()].REV_OFFSET));
    else
#endif
      return (unsigned char*) GetConcreteElement(el.GetHandle()) +
        m_ST[el.GetType()].REV_OFFSET;
  ...
  */
  ...
  void ReplaceNode(TopNode node, TopNode newnode, int num_elems, TopElement* elems);
  ...
  *************************/
  ...
  void UpdateRevAdj(const TopElement& el, int fi); 
  ...
  void MakeDiamond(TopFacet f, TopFacetUse* fu_out, TopFacetUse* fum_out);
  ...
  void SplitFacetNotifySplitEdge(
    TopEdge e, TopEdge em, TopElement coh, TopElement sel[6]);
  ...
  void SwapFacetNotifySwapEdge(TopEdge e, TopElement sel[6]);
...
public:
  ...
  *************************/
  ...
  {
    SetAdjNoUpdateRevAdj(el, fi, adj);
    UpdateRevAdj(el, fi);
  }

  void SetAdjNoUpdateRevAdj(const TopElement& el, int fi, TopElement adj)
  {
#ifdef TOP_USE_RESERVOIR_OPTM
    if (m_is_reservoir) {
      if (adj.IsValid())
        *(GetAdjArrayReservoir(el)) |= (unsigned char) 1 << fi;
      else
        *(GetAdjArrayReservoir(el)) &= ~((unsigned char) 1 << fi);
    } else
#endif
    GetAdjArray(el)[fi] = adj;
  }
  ...
  TopElement GetAdj(const TopElement& el, int fi)
  {
#ifdef TOP_USE_RESERVOIR_OPTM
    if (m_is_reservoir) {
      if (HasAdj(el, fi))
  ...
  **********************/
  ...
  TopFacetUse GetFacetUse(const TopElement& el, int i)
  {
  ...
  */
  TopFacetUse GetMate(const TopFacetUse& fu)
  {
    TopElement el  = GetElem(fu);
    TopElement adj = GetAdj(el, fu.GetLocalId());
    if (adj.IsValid()) {
  #ifdef TOP_USE_REV_ADJ
      return TopFacetUse(adj.GetHandle(), 
        GetRevAdjArray(el)[fu.GetLocalId()] & TOP_REV_ADJ_FACET_PAT);
  #else
  ...
  TopEdgeUse GetEdgeUse(const TopFacetUse& fu, int ei) 
  { 
  ...
  TopVertexUse GetVertexUse(const TopFacetUse& fu, int vi)
  {
  ...
  ************************/
  ...
  TopFaceUse GetMate(const TopFaceUse& fu)
  {
    ...
    if (GetDimension(el) == 2 || !adj.IsValid())
      return TopFaceUse();
  #ifdef TOP_USE_REV_ADJ
    return TopFaceUse(adj.GetHandle(), 
      GetRevAdjArray(el)[fu.GetLocalId()] & TOP_REV_ADJ_FACET_PAT);
  #else
  ...
  */
  ...
  /**
    ...
    local orientation.
  */
  TopFacetUse GetFacetUse(const TopEdgeUse& eu, int i)
  {
  ...
  /**
    ...
    @see TopModel::UpdateAdj()
  */
  TopEdgeUse GetMate(const TopEdgeUse& eu, int i)
  {
    TopElement          el = GetElem(eu);
    int                 ei = eu.GetLocalId();
    int                 fi = E_F(el, ei, i);
    TopElement          adj = GetAdj(el, fi);
  #ifdef TOP_USE_REV_ADJ
    if (adj.IsValid()) {
      unsigned char* rev = GetRevAdjArray(el);
      // Supposing that, for a facet-use, there is always an edge-use
      // following ith vertex of the facet-use (like a half-edge)...
//TODO: assumes a maximum of 4 local id bits for facets...
      return GetEdgeUse(GetFacetUse(adj, rev[fi] & TOP_REV_ADJ_FACET_PAT),
          ((rev[fi] >> TOP_REV_ADJ_FACET_BITS) + NUM_F_V(el, fi) - E_F_E(el, ei, i) - 1) % 
              NUM_F_V(el, fi)); 
    } else
      return TopEdgeUse();
  #else
  ...
  TopNode GetVertexNode(const TopEdgeUse& eu, int vi)
  {
    return GetNodeArray(GetElem(eu))[
  ...
  bool CompareVertices(const TopEdgeUse& eu1, const TopEdgeUse& eu2);
  ...
  ************************/
  ...
  /**
    ...
    @see TopModel::UpdateAdj()
  */
  TopVertexUse GetMate(const TopVertexUse& vu, int i)
  {
    TopElement          el = GetElem(vu);
    ...
    if (adj.IsValid()) {
      unsigned char* rev = GetRevAdjArray(el);
//TODO: assumes a maximum of 4 local id bits for facets...
      return GetVertexUse(GetFacetUse(adj, rev[fi] & TOP_REV_ADJ_FACET_PAT),
        ((rev[fi] >> TOP_REV_ADJ_FACET_BITS) + 
          NUM_F_V(el, fi) - V_F_V(el, vi, i)) % NUM_F_V(el, fi)); 
    } else
      return TopVertexUse();
  #else
    if (adj.IsValid()) {
  ...
  ************************/
  ...
  #else
    return TopVertexUse(v.GetHandle());
  #endif
  }
  ...
  */
  ...
  /**
   * Replaces all nodes 'ni' of element 'el' with nodes newnodes[ni].
   * Updates the structure accordingly.
   */
  void ReplaceNodes(TopElement el, const TopNode* newnodes);
  ...
  /**
    ...
    @see TopModelEditListener
  */
  void CollapseEdge(TopEdge e, TopNode newnode);
  ...
  */
  bool IsEdgeCollapseValid(TopEdge e);

  /**
    Returns whether the last edge collapsed has left the mesh 
    in a valid (manifold) state.
  */
  bool IsEdgeCollapseManifold(TopNode newnode);

  /**
      Splits a node into two new nodes. The given node is replaced
    by newnode1 and newnode2 in the given elements, and thus leaves
    a "hole" where new elements must be inserted by the user.
      This method notifies of event ReplaceElemNode().
      Currently, only T3 and TET4 meshes are supported and the same
    assumptions as CollapseEdge() are made.
    @see CollapseEdge()
    @see TopModelEditListener
  */
  void SplitNode(TopNode node, TopNode newnode1, TopNode newnode2,
    int num_replaces_n1, int num_replaces_n2, 
    TopElement* replace_el_n1, TopElement* replace_el_n2); 

  void UndoableCollapseEdge(TopEdge e_old, double x, double y, double z);
  void UndoCollapseEdge();

  /** DEPRECATED */
  /** Sets a listener for model edition events. */
  void SetEditListener(TopModelEditListener* listener)
  {
    m_edit_notifier.ClearEditListeners();
    m_edit_notifier.AddEditListener(listener);
  }

  /** DEPRECATED */
  TopModelEditListener* GetEditListener()
  {
    if (m_edit_notifier.GetEditListeners()->Size() > 0)
      return m_edit_notifier.GetEditListeners()->Get(0);
    else
      return NULL;
  }
...
//TODO  
//  void GetNodeDenseAttribId(const char* name);
...
//TODO  
//  void GetElemDenseAttribId(int elemtype, const char* name);
...
//TODO  
//  void GetNodeSparseAttribId(const char* name);
...
private:
  ...
  bool EdgeCollapseWillAnyMergingElemsHaveSameNodes(
      const TopEdge& collapsing_edge);
