...
typedef void (*TopSplitEdgeCb)(TopModel* m, TopEdge old_e, TopEdge new_e[2], void* data);
typedef void (*TopSwapEdgeCb)(TopModel* m, TopEdge old_e, TopEdge new_e, void* data);
...
//typedef void (*TopInsertVertexCb)(TopModel* m, TopVertex new_v, void* data);
//typedef void (*TopInsertEdgeCb)(TopModel* m, TopEdge new_e, void* data);
...
TOPS_API void topModel_SetSplitEdgeCb(TopModel* m, TopSplitEdgeCb cb, void* data);
TOPS_API void topModel_SetSwapEdgeCb(TopModel* m, TopSwapEdgeCb cb, void* data);
...
//TOPS_API void topModel_SetInsertVertexCb(TopModel* m, TopInsertVertexCb cb, void* data);
//TOPS_API void topModel_SetInsertEdgeCb(TopModel* m, TopInsertEdgeCb cb, void* data);
...
/**  
  ...
  TopSplitEdgeCb(), TopInsertNodeCb(), TopInsertVertexCb(), TopInsertEdgeCb(). 
    When TopSplitElemCb() and TopSplitEdgeCb() are called, the old element is 
  ...
  and vertices are not guaranteed. However, in TopSplitEdgeCb(), retrieving the
...
/**
  ...
  two non-cohesive elements.
    The swap modifications are notified by the callbacks: TopSwapElemCb() and 
  TopSwapEdgeCb().
    When TopSwapElemCb() and TopSwapEdgeCb() are called, the old element is 
  already disconnect from the model. Thus, only the local entities of this 
  element (facet-use, edge-use and vertex-use) can be retrieved. Facets, edges 
  and vertices are not guaranteed. However, in TopSwapEdgeCb(), retrieving the
  facet associated to the edge is allowed. 
    The main uses for the above callbacks are to move attributes from the old 
  entities to the new ones.
