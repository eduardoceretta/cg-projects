...
public:
  ...
  */
   ...
   */
  static void InterpolatePoint (int topology, float* nodepos, 
...
private:
  ...
  void CreateRefinedElems (
      TopElement elem, int nx, int ny, int nz, 
      TopRawArray<TopNode>& childnodes, bool update_adj);

  void CreateRefinedNodes (
      TopElement elem, int nx, int ny, int nz, 
      TopRawArray<TopNode>& o_childnodes);

  void ReuseRefinedNodesOfAdjFaces (
      TopElement elem, int nx, int ny, int nz,
      TopRawArray<TopNode>& o_childnodes);

  void ReuseRefinedNodesOfAdjEdges (
      TopElement elem, int nx, int ny, int nz,
      TopRawArray<TopNode>& o_childnodes);

  void ReuseRefinedNodesOfAdjVertices (
      TopElement elem, int nx, int ny, int nz,
      TopRawArray<TopNode>& o_childnodes);

  void DetermineNodesOfElemInGrid (int topology, int nx, int ny, int nz, 
                                   int i, int j, int k, 
                                   const TopNode* nodes, 
  ...
  void GetChildNodeGridDimensions (TopElement elem, int nx, int ny, int nz,
                                   int* o_node_nx, int* o_node_ny, int* o_node_nz);
  ...
  void GetNodePositionInRefinedGrid (int topology, int nx, int ny, int nz, 
                                     TopVertexUse vu,
                                     int* o_child_node_i, 
                                     int* o_child_node_j, 
                                     int* o_child_node_k);
  ...
  void NodePositionInFaceRefinementToGrid (int topology, int nx, int ny, int nz,
